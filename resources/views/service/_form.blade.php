    ...
    <script>
        $( document ).ready(function() {
            ...
            $('#add_qualification').on("click", function () {
                ...
                prot += '<option value="null">-- Bitte wählen --</option> @foreach($users as $user) <option @if(in_array($service->id, $user->services_inHolidayList())) data-icon="glyphicon glyphicon-remove" data-subtext="Abwesend" @endif value="{{$user->id}}">{{$user->first_name}} {{$user->name}}</option> @endforeach';
                prot += '</select> </td>';
